...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagYaobi)
    {
        info->event[info->num] = EVENT_Yaobi;
        info->num++;
        dataPoints->valueYaobi = gizStandardDecompressionValue(Yaobi_BYTEOFFSET,Yaobi_BITOFFSET,Yaobi_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagY_qianjin)
    {
        info->event[info->num] = EVENT_Y_qianjin;
        info->num++;
        dataPoints->valueY_qianjin = gizStandardDecompressionValue(Y_qianjin_BYTEOFFSET,Y_qianjin_BITOFFSET,Y_qianjin_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagY_houtui)
    {
        info->event[info->num] = EVENT_Y_houtui;
        info->num++;
        dataPoints->valueY_houtui = gizStandardDecompressionValue(Y_houtui_BYTEOFFSET,Y_houtui_BITOFFSET,Y_houtui_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagYeya)
    {
        info->event[info->num] = EVENT_Yeya;
        info->num++;
        dataPoints->valueYeya = gizStandardDecompressionValue(Yeya_BYTEOFFSET,Yeya_BITOFFSET,Yeya_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagX_qianjin)
    {
        info->event[info->num] = EVENT_X_qianjin;
        info->num++;
        dataPoints->valueX_qianjin = gizStandardDecompressionValue(X_qianjin_BYTEOFFSET,X_qianjin_BITOFFSET,X_qianjin_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagX_houtui)
    {
        info->event[info->num] = EVENT_X_houtui;
        info->num++;
        dataPoints->valueX_houtui = gizStandardDecompressionValue(X_houtui_BYTEOFFSET,X_houtui_BITOFFSET,X_houtui_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLED_ON)
    {
        info->event[info->num] = EVENT_LED_ON;
        info->num++;
        dataPoints->valueLED_ON = gizStandardDecompressionValue(LED_ON_BYTEOFFSET,LED_ON_BITOFFSET,LED_ON_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLED_OFF)
    {
        info->event[info->num] = EVENT_LED_OFF;
        info->num++;
        dataPoints->valueLED_OFF = gizStandardDecompressionValue(LED_OFF_BYTEOFFSET,LED_OFF_BITOFFSET,LED_OFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagY_qianjin_value)
    {
        info->event[info->num] = EVENT_Y_qianjin_value;
        info->num++;
        dataPoints->valueY_qianjin_value = gizX2Y(Y_qianjin_value_RATIO,  Y_qianjin_value_ADDITION, exchangeBytes(issuedData->attrVals.valueY_qianjin_value));
    }
        
    if(0x01 == issuedData->attrFlags.flagY_houtui_value)
    {
        info->event[info->num] = EVENT_Y_houtui_value;
        info->num++;
        dataPoints->valueY_houtui_value = gizX2Y(Y_houtui_value_RATIO,  Y_houtui_value_ADDITION, exchangeBytes(issuedData->attrVals.valueY_houtui_value));
    }
        
    if(0x01 == issuedData->attrFlags.flagX_qianjin_value)
    {
        info->event[info->num] = EVENT_X_qianjin_value;
        info->num++;
        dataPoints->valueX_qianjin_value = gizX2Y(X_qianjin_value_RATIO,  X_qianjin_value_ADDITION, exchangeBytes(issuedData->attrVals.valueX_qianjin_value));
    }
        
    if(0x01 == issuedData->attrFlags.flagX_houtui_value)
    {
        info->event[info->num] = EVENT_X_houtui_value;
        info->num++;
        dataPoints->valueX_houtui_value = gizX2Y(X_houtui_value_RATIO,  X_houtui_value_ADDITION, exchangeBytes(issuedData->attrVals.valueX_houtui_value));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueYaobi != cur->valueYaobi)
    {
        GIZWITS_LOG("valueYaobi Changed\n");
        ret = 1;
    }
    if(last->valueY_qianjin != cur->valueY_qianjin)
    {
        GIZWITS_LOG("valueY_qianjin Changed\n");
        ret = 1;
    }
    if(last->valueY_houtui != cur->valueY_houtui)
    {
        GIZWITS_LOG("valueY_houtui Changed\n");
        ret = 1;
    }
    if(last->valueYeya != cur->valueYeya)
    {
        GIZWITS_LOG("valueYeya Changed\n");
        ret = 1;
    }
    if(last->valueX_qianjin != cur->valueX_qianjin)
    {
        GIZWITS_LOG("valueX_qianjin Changed\n");
        ret = 1;
    }
    if(last->valueX_houtui != cur->valueX_houtui)
    {
        GIZWITS_LOG("valueX_houtui Changed\n");
        ret = 1;
    }
    if(last->valueLED_ON != cur->valueLED_ON)
    {
        GIZWITS_LOG("valueLED_ON Changed\n");
        ret = 1;
    }
    if(last->valueLED_OFF != cur->valueLED_OFF)
    {
        GIZWITS_LOG("valueLED_OFF Changed\n");
        ret = 1;
    }
    if(last->valueY_qianjin_value != cur->valueY_qianjin_value)
    {
        GIZWITS_LOG("valueY_qianjin_value Changed\n");
        ret = 1;
    }
    if(last->valueY_houtui_value != cur->valueY_houtui_value)
    {
        GIZWITS_LOG("valueY_houtui_value Changed\n");
        ret = 1;
    }
    if(last->valueX_qianjin_value != cur->valueX_qianjin_value)
    {
        GIZWITS_LOG("valueX_qianjin_value Changed\n");
        ret = 1;
    }
    if(last->valueX_houtui_value != cur->valueX_houtui_value)
    {
        GIZWITS_LOG("valueX_houtui_value Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Yaobi_BYTEOFFSET,Yaobi_BITOFFSET,Yaobi_LEN,(uint8_t *)devStatusPtr,dataPoints->valueYaobi);
    gizStandardCompressValue(Y_qianjin_BYTEOFFSET,Y_qianjin_BITOFFSET,Y_qianjin_LEN,(uint8_t *)devStatusPtr,dataPoints->valueY_qianjin);
    gizStandardCompressValue(Y_houtui_BYTEOFFSET,Y_houtui_BITOFFSET,Y_houtui_LEN,(uint8_t *)devStatusPtr,dataPoints->valueY_houtui);
    gizStandardCompressValue(Yeya_BYTEOFFSET,Yeya_BITOFFSET,Yeya_LEN,(uint8_t *)devStatusPtr,dataPoints->valueYeya);
    gizStandardCompressValue(X_qianjin_BYTEOFFSET,X_qianjin_BITOFFSET,X_qianjin_LEN,(uint8_t *)devStatusPtr,dataPoints->valueX_qianjin);
    gizStandardCompressValue(X_houtui_BYTEOFFSET,X_houtui_BITOFFSET,X_houtui_LEN,(uint8_t *)devStatusPtr,dataPoints->valueX_houtui);
    gizStandardCompressValue(LED_ON_BYTEOFFSET,LED_ON_BITOFFSET,LED_ON_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED_ON);
    gizStandardCompressValue(LED_OFF_BYTEOFFSET,LED_OFF_BITOFFSET,LED_OFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED_OFF);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));


    devStatusPtr->valueY_qianjin_value = exchangeBytes(gizY2X(Y_qianjin_value_RATIO,  Y_qianjin_value_ADDITION, dataPoints->valueY_qianjin_value)); 
    devStatusPtr->valueY_houtui_value = exchangeBytes(gizY2X(Y_houtui_value_RATIO,  Y_houtui_value_ADDITION, dataPoints->valueY_houtui_value)); 
    devStatusPtr->valueX_qianjin_value = exchangeBytes(gizY2X(X_qianjin_value_RATIO,  X_qianjin_value_ADDITION, dataPoints->valueX_qianjin_value)); 
    devStatusPtr->valueX_houtui_value = exchangeBytes(gizY2X(X_houtui_value_RATIO,  X_houtui_value_ADDITION, dataPoints->valueX_houtui_value)); 
